import { createClient, sql, VercelPoolClient } from "@vercel/postgres";
import {
  CustomerField,
  CustomersTableType,
  InvoiceForm,
...
export async function fetchRevenue() {
  try {
    ...
    const data = await (await db.connect()).sql<Revenue>`SELECT * FROM revenue`;
...
export async function fetchCardData() {
  try {
    ...
    const invoiceCountPromise = (await db.connect())
      .sql`SELECT COUNT(*) FROM invoices`;
    const customerCountPromise = (await db.connect())
      .sql`SELECT COUNT(*) FROM customers`;
    const invoiceStatusPromise = (await db.connect()).sql`SELECT
